...
# moving factors round
test2$lex_typicality = factor(test2$lex_typicality, levels = c('typical', 'random', 'atypical'))
test2$reg_rate = factor(test2$reg_rate, levels = c('nc', '-40%', '+40%'))
test2$baseline_av_z = scales::rescale(test2$baseline_av)
test2$pre_reg_av_z = scales::rescale(test2$pre_reg_av)
test2$baseline_av_c = test2$baseline_av - mean(test2$baseline_av)
test2$pre_reg_av_c = test2$pre_reg_av - mean(test2$pre_reg_av)

# lex typ: typical - random - reversed
# reg rate: nc - -40% - +40%
test2$lexical_typicality = test2$lex_typicality
test2$regularisation_rate = ifelse(
  as.character(test2$reg_rate) == 'nc', 'no change',
  as.character(test2$reg_rate)
  )
test2$regularisation_rate = factor(test2$regularisation_rate, levels = c('no change', '-40%', '+40%'))
...
t1 %>% 
  ...
  mutate(
    ...
    case_when(
      ...
      term == 'verb_baseline_average:bot_lexical_typicalityatypical' ~ 'verb baseline average : bot lexical typicality = atypical',
