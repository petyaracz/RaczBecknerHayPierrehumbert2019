...
lme.fit2 = glmer(resp_post_reg ~ 1 + baseline_av_c * lexical_typicality + pre_reg_av_c + regularisation_rate +
    (1 + baseline_av_c | participant_id) + 
    (1  | word), 
  data = test2, 
  family = binomial (link = logit),
...
plot(effects::effect('regularisation_rate', lme.fit2), main = '', xlab = 'regularisation shift', ylab = 'regular response in post-test')
plot(effects::effect('baseline_av_c:lexical_typicality', lme.fit2), multiline = T, lines=list(lty=c(1,2,3)), main = '', xlab = 'verb baseline mean', ylab = 'regular response in post-test')

# https://arxiv.org/abs/1507.04544 s5.2
```

Let's have a few estimates for CI-s.
...
confint(lme.fit2, method = 'Wald', level = 0.95)
effects::allEffects(lme.fit2)
