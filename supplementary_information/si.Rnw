...
library(broom.mixed) # for tidy summaries, lmer
library(ggthemes) # for colourblind scale

# functions

# takes lmer fit, spits out coef table with ast
niceCoefTable = function(fit){
  my_table = fit %>% 
    tidy %>% 
    dplyr::filter(
      effect == 'fixed'
    ...
    dplyr::rename(`Z score` = statistic) %>% 
    select(
      - effect,
      - p.value,
      - group
...
# takes lmer fit, spits out vif for coefs
...
baseline = baseline %>% 
  dplyr::mutate(
    rescaled_CELEX_mgl_feature_distance = scales::rescale(baseline_mgl_features), # rescale regularity weights
    rescaled_CELEX_gcm_feature_distance = scales::rescale(baseline_gcm_features),
  )
...
test2 = test2 %>% 
  dplyr::mutate(
    ...
    co_player_lexical_typicality = dplyr::case_when( # set factor levels used in paper, order factor
    ...
    co_player_regularisation_rate = reg_rate %>% # order factor
      factor(levels = c('nc', '-40%', '+40%')),
    rescaled_CELEX_mgl_feature_distance = scales::rescale(baseline_mgl_features), # scale regular weights
    rescaled_CELEX_gcm_feature_distance = scales::rescale(baseline_gcm_features),
    rescaled_individual_mgl_feature_distance = scales::rescale(individual_mgl_features),
    rescaled_individual_gcm_feature_distance = scales::rescale(individual_gcm_features),
    gcm_score_diff = rescaled_individual_gcm_feature_distance - rescaled_CELEX_gcm_feature_distance, # calculate score diff
    mgl_score_diff = rescaled_individual_mgl_feature_distance - rescaled_CELEX_mgl_feature_distance
  )
...
mgl_baseline_rules_raw = readr::read_tsv('~/Github/RaczBecknerHayPierrehumbert2019/models/mgl/baseline_mgl/CELEXFull3.sum') %>% 
  dplyr::mutate(
    ...
    form2 = stringr::str_replace_all(form2, 'o', '5') # transcription thing
  )
...
print(xtable::xtable(gcm_table, digits=2, include.rownames = FALSE, label="gcmsetsize", caption="Number of forms in each verb class, GCM training data"), include.rownames = F)
...
splive_gcm = test2 %>% 
  ...
  dplyr::pull(rescaled_CELEX_gcm_feature_distance) %>% 
  unique %>% 
  round(2)
...
# we also want the TOTAL number of related forms and exceptions across rules.

mgl_count_total1 = mgl_count %>% 
  dplyr::select(related_list) %>% 
  tidyr::unnest(related_list) %>% # we now have the related forms as one verb per line, which is tidy. but the verbs repeat, since the same verb can be a related form for several target verbs
  dplyr::distinct() %>% # so we distinct them!
  dplyr::mutate(
    type = 'related forms' # by the way, this is related forms
...
mgl_count_total2 = mgl_count %>% 
  dplyr::select(exception_list) %>% 
  tidyr::unnest(exception_list) %>% # we now have the related forms as one verb per line, which is tidy. but the verbs repeat, since the same verb can be a related form for several target verbs
  dplyr::distinct() %>% # so we distinct them!
  dplyr::mutate(
    type = 'exceptions' # by the way, this is related forms
  ) %>% 
  dplyr::rename(
   forms = exception_list 
  )

all_forms_n_mgl = mgl_count_total2 %>% 
  dplyr::bind_rows(mgl_count_total1) %>% 
  dplyr::distinct(forms) %>% 
  dplyr::summarise(n =n()) %>% 
  dplyr::pull(n)

@

Note that the sets of exceptions and related forms of each rule can overlap.  As a consequence, the MGL rules apply to fewer forms than apparent from the table: \Sexpr{all_forms_n_mgl} in total.

<<mgltable, echo=F, eval=T, results=tex>>=
print(xtable::xtable(mgl_table, digits=2, label="mglsetsize", caption="Number of forms in each verb class, MGL training data"), include.rownames = F)
...
# regular
splive_mgl_rules[splive_mgl_rules$B == 'd',]$Pfeat
splive_mgl_rules[splive_mgl_rules$B == 'd',]$hits
splive_mgl_rules[splive_mgl_rules$B == 'd',]$scope
splive_mgl_rules[splive_mgl_rules$B == 'd',]$reliability
splive_mgl_rules[splive_mgl_rules$B == 'd',]$confidence

# irregular
splive_mgl_rules[splive_mgl_rules$B == 'o',]$Pfeat # this kept the original transcription that we change to DISC 5 in form2
splive_mgl_rules[splive_mgl_rules$B == 'o',]$hits
splive_mgl_rules[splive_mgl_rules$B == 'o',]$scope
splive_mgl_rules[splive_mgl_rules$B == 'o',]$`related forms`
splive_mgl_rules[splive_mgl_rules$B == 'o',]$exceptions
splive_mgl_rules[splive_mgl_rules$B == 'o',]$reliability
splive_mgl_rules[splive_mgl_rules$B == 'o',]$confidence


test2 %>% 
  dplyr::filter(word == 'splive') %>% 
  dplyr::select(baseline_mgl_features,rescaled_CELEX_mgl_feature_distance) %>% 
  dplyr::distinct()
...
# we get a total rule count as well. since rules overlap, this isn't a sum of the rule counts above

total_rules = mgl_baseline_rules_raw %>% # take raw rules
  dplyr::filter( # throw out non-category verbs and non reg/irreg rules (in our definition)
    !is.na(category),
    !is.na(rule.type)
  ) %>% 
...
lme.fit1 = glmer(resp_post_reg ~ 
    1 + verb_baseline_average * co_player_lexical_typicality + participant_prereg_average + co_player_regularisation_rate +
...
lme.fit1c = glmer(resp_post_reg ~ 
    1 + verb_baseline_average * co_player_lexical_typicality * co_player_regularisation_rate + participant_prereg_average + 
    (1 + verb_baseline_average | participant_id) + 
    (1  | word), 
  data = test2, 
  family = binomial (link = logit),
...
# here are the effect estimate figures from the paper:
plot(effects::effect('co_player_regularisation_rate', lme.fit1), xlab = 'co-player regularization shift', ylab = 'player post-test regularization', main = 'Effect of co-player rate of regularization on post-test responses', lwd = 2)

plot(effects::effect('verb_baseline_average:co_player_lexical_typicality', lme.fit1), multiline = T, lines=list(lty=c(1,2,3)), lwd = 2, xlab = 'verb baseline average', ylab = 'player post-test regularization', main = 'Effect of co-player lexical typicality on post-test responses')
...
load('~/Github/RaczBecknerHayPierrehumbert2019/supplementary_information/models/si_fit1.rda')
lme.fit1.sum = niceCoefTable(lme.fit1)

print(xtable::xtable(lme.fit1.sum, digits = 2, label = 'regressiontable1', caption = 'Experimental Data, Post-test Regression Model Summary. Model formula: post-test regular response ~ verb baseline mean x lexical typicality + participant pretest mean + regularization shift + (1 + verb.baseline.mean | participant) + (1 | verb)'), include.rownames = F)
@
...
test2b = test2 %>% 
  dplyr::filter(
    co_player_lexical_typicality != 'typical'
  )
...
C_CELEX_mgl_features = with(baseline, Hmisc::somers2(rescaled_CELEX_mgl_feature_distance,regular)['C']) %>% 
  round(3)
C_CELEX_gcm_features = with(baseline, Hmisc::somers2(rescaled_CELEX_gcm_feature_distance,regular)['C']) %>% 
  round(3)

# get correlation between mgl and gcm scores in baseline
mgl_gcm_cor = baseline %>% 
  dplyr::distinct(word,rescaled_CELEX_mgl_feature_distance,baseline_gcm_features) %>% 
  dplyr::summarise(
    cor = cor(rescaled_CELEX_mgl_feature_distance,baseline_gcm_features)
  ) %>% 
  dplyr::pull() %>% 
  round(3)
...
baseline_words = baseline %>% 
  dplyr::distinct(word, rescaled_CELEX_mgl_feature_distance, rescaled_CELEX_gcm_feature_distance)

# fitting models 1 and 2 on it
celex_m1 = lm(rescaled_CELEX_gcm_feature_distance ~ 1 + rescaled_CELEX_mgl_feature_distance, data = baseline_words)
celex_m2 = lm(rescaled_CELEX_mgl_feature_distance ~ 1 + rescaled_CELEX_gcm_feature_distance, data = baseline_words)

# getting residuals from models and putting them back in this data frame
baseline_words = baseline_words %>% 
  dplyr::mutate(
    rescaled_CELEX_gcm_feature_distance_residual = resid(celex_m1),
    rescaled_CELEX_mgl_feature_distance_residual = resid(celex_m2)
  )

# joining it with entire baseline data
baseline = left_join(baseline, baseline_words, by = c("word", "rescaled_CELEX_mgl_feature_distance", "rescaled_CELEX_gcm_feature_distance"))

# fitting models 3 and 4
celex_m3 = glmer(regular ~ 
                1 + rescaled_CELEX_mgl_feature_distance + rescaled_CELEX_gcm_feature_distance_residual + 
                (1 + rescaled_CELEX_mgl_feature_distance + rescaled_CELEX_gcm_feature_distance_residual | subject), 
                data = baseline, 
                family = binomial ( link = logit ),
                control=glmerControl(optimizer="bobyqa"))

celex_m4 = glmer(regular ~ 
                1 + rescaled_CELEX_gcm_feature_distance + rescaled_CELEX_mgl_feature_distance_residual + 
                (1 + rescaled_CELEX_gcm_feature_distance + rescaled_CELEX_mgl_feature_distance_residual | subject), 
                data = baseline, 
                family = binomial ( link = logit ),
                control=glmerControl(optimizer="bobyqa"))
...
celex_m3_summary = niceCoefTable(celex_m3)
print(xtable::xtable(celex_m3_summary, digits = 2, label = 'resid1', caption = 'Model 3: MGL scores and GCM residuals, baseline data'), include.rownames = F)
@

<<celex_resid2, echo=F,eval=T, results=tex>>=
load('models/si_celex_m4.rda')
celex_m4_summary = niceCoefTable(celex_m4)
print(xtable::xtable(celex_m4_summary, digits = 2, label = 'resid2', caption = 'Model 4: GCM scores and MGL residuals, baseline data'), include.rownames = F)
@
...
# C_CELEX_mgl_features # celex-mgl on baseline
# C_CELEX_gcm_features # celex-gcm on baseline

C_test2_baseline_mgl_features = with(test2, Hmisc::somers2(rescaled_CELEX_mgl_feature_distance,resp_post_reg)['C']) %>% round(3)
C_test2_baseline_gcm_features = with(test2, Hmisc::somers2(rescaled_CELEX_gcm_feature_distance,resp_post_reg)['C']) %>% round(3)
C_test2_individual_mgl_features = with(test2, Hmisc::somers2(rescaled_individual_mgl_feature_distance,resp_post_reg)['C']) %>% round(3)
C_test2_individual_gcm_features = with(test2, Hmisc::somers2(rescaled_individual_gcm_feature_distance,resp_post_reg)['C']) %>% round(3)
...
\begin{table}[h]
    ...
    \begin{tabular}{lllll}
            ...
            \hline
\textsc{Celex}-GCM & real verbs & `analogy' & verbs in baseline &\Sexpr{C_CELEX_gcm_features }\\
\textsc{Celex}-MGL & real verbs & `rules' & verbs in baseline &\Sexpr{C_CELEX_mgl_features }\\ \hline
...
indiv_m5 = glmer(resp_post_reg ~
    1 + rescaled_CELEX_gcm_feature_distance + gcm_score_diff + participant_prereg_average + 
    (1 + rescaled_CELEX_gcm_feature_distance + gcm_score_diff | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))
...
indiv_m6 = glmer(resp_post_reg ~ 
    1 + rescaled_CELEX_mgl_feature_distance + mgl_score_diff + participant_prereg_average  + 
    (1 + rescaled_CELEX_mgl_feature_distance + mgl_score_diff | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))
...
indiv_m5_sum = niceCoefTable(indiv_m5)
print(xtable::xtable(indiv_m5_sum, digits = 2, label = 'indiv1', caption = 'Model 5: Celex-GCM and individual score difference, post-test data'), include.rownames = F)

@

<<indiv_diff2, echo=F,eval=T, results=tex>>=
load('models/si_indiv_m6.rda')
indiv_m6_sum = niceCoefTable(indiv_m6)
print(xtable::xtable(indiv_m6_sum, digits = 2, label = 'indiv2', caption = 'Model 6: Celex-MGL and individual score difference, post-test data'), include.rownames = F)
@
...
comp_fit1 = glmer(resp_post_reg ~ 
    1 + rescaled_CELEX_mgl_feature_distance + rescaled_individual_gcm_feature_distance + participant_prereg_average  + 
    (1 + rescaled_CELEX_mgl_feature_distance + rescaled_individual_gcm_feature_distance | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))
...
comp_fit1b = glmer(resp_post_reg ~ 
    1 + rescaled_individual_gcm_feature_distance + participant_prereg_average  + 
    (1 + rescaled_individual_gcm_feature_distance | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))
...
comp_fit1c = glmer(resp_post_reg ~ 
    1 + rescaled_CELEX_mgl_feature_distance + participant_prereg_average  + 
    (1 + rescaled_CELEX_mgl_feature_distance | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))
...
comp_fit1d = glmer(resp_post_reg ~ 
    1 + rescaled_CELEX_mgl_feature_distance + rescaled_individual_gcm_feature_distance + rescaled_individual_mgl_feature_distance + participant_prereg_average  + 
    (1 + rescaled_CELEX_mgl_feature_distance + rescaled_individual_gcm_feature_distance | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))
...
comp_fit1e = glmer(resp_post_reg ~ 
    1 + rescaled_CELEX_mgl_feature_distance + rescaled_individual_gcm_feature_distance + rescaled_CELEX_gcm_feature_distance + participant_prereg_average  + 
    (1 + rescaled_CELEX_mgl_feature_distance + rescaled_individual_gcm_feature_distance | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))
...
p3 = anova(comp_fit1, comp_fit1d)[2,8] %>% round(2) # with individual mgl
p4 = anova(comp_fit1, comp_fit1e)[2,8] %>% round(2) # with celex gcm


@

<<comparison_model3, echo=F,eval=T, results=tex>>=
comp_fit1_sum = niceCoefTable(comp_fit1)
print(xtable::xtable(comp_fit1_sum, digits = 2, label = 'comp3', caption = 'Celex-MGL and Individual-GCM predict variation in the post-test'), include.rownames = F)
@

Goodness-of-fit tests show that model fit is significantly worse if we exclude {\sc Celex}-MGL ($\chi^2 = $ \Sexpr{chisq1} ***) or individual-GCM ($\chi^2 = $ \Sexpr{chisq2} ***) as predictors. It is not significantly better if we include individual-MGL ($\chi^2 = $ \Sexpr{chisq3}, p = \Sexpr{p3}) or {\sc Celex}-GCM ($\chi^2 = $ \Sexpr{chisq4}, p = \Sexpr{p4}).
...
second_panel = test2 %>% 
  dplyr::group_by(
    word,
    co_player_lexical_typicality,
    rescaled_CELEX_gcm_feature_distance
  ) %>% 
  dplyr::rename(
    baseline = rescaled_CELEX_gcm_feature_distance
  ) %>% 
  dplyr::summarise(
    posttest = mean(rescaled_individual_gcm_feature_distance)
  ) %>% 
  dplyr::mutate(
    type = 'GCM'
  )
...
third_panel = test2 %>% 
  dplyr::group_by(
    word,
    co_player_lexical_typicality,
    rescaled_CELEX_mgl_feature_distance
  ) %>% 
  dplyr::rename(
    baseline = rescaled_CELEX_mgl_feature_distance
  ) %>% 
  dplyr::summarise(
    posttest = mean(rescaled_individual_mgl_feature_distance)
  ) %>% 
  dplyr::mutate(
    type = 'MGL'
  )
...
three_studies = bind_rows(first_panel,second_panel,third_panel) %>% 
  dplyr::mutate(
    ...
    linetype = case_when(
      co_player_lexical_typicality == 'typical' ~ 'solid',
      co_player_lexical_typicality == 'random' ~ 'dotdash',
      co_player_lexical_typicality == 'reversed' ~ 'twodash'
    )
  )
...
# add loess smooths.
p1 = ggplot(three_studies, aes(x = baseline, y = posttest, colour = co_player_lexical_typicality, shape = co_player_lexical_typicality, linetype = linetype)) +
  guides(size = F) +
  geom_point(alpha = .25) +
  geom_smooth(se = F, method = 'loess', formula = 'y ~ x') +
  facet_wrap( ~ type, ncol = 2) +
  scale_shape_discrete(name="Lexical\ntypicality", solid = FALSE) +
...
ggsave('si_figure.pdf', plot = p1, width = 9, height = 7)
