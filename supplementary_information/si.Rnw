...
library(xtable)
library(lme4)
library(patchwork) # take this out
...
test2 = test2 %>% 
  mutate(
    ...
    bot_lexical_typicality = case_when( # set factor levels used in paper, order factor
...
save(lme.fit1b, file = 'si_fit1b.rda')
save(lme.fit1c, file = 'si_fit1c.rda')
vif.mer(lme.fit1)
vif.mer(lme.fit1c)
...
# summary(fit_power1)
# summary(fit_power2)
save(fit_power1, file = 'si_fit_power1.rda')
save(fit_power2, file = 'si_fit_power2.rda')
...
save(comp_fit1, file = 'si_comp_fit1.rda')
save(comp_fit1b, file = 'si_comp_fit1b.rda')
save(comp_fit1c, file = 'si_comp_fit1c.rda')
save(comp_fit1d, file = 'si_comp_fit1d.rda')
save(comp_fit1e, file = 'si_comp_fit1e.rda')
...
load('si_comp_fit1.rda')
load('si_comp_fit1b.rda')
load('si_comp_fit1c.rda')
load('si_comp_fit1d.rda')
load('si_comp_fit1e.rda')
...
print(xtable(summary(comp_fit1)$coef, digits = 3, label = 'comp3', caption = 'Celex-MGL and Individual-GCM predict variation in the post-test'))
@
...
first_panel = test2 %>% 
  ...
  summarise(
    post_reg_mean = post_reg_av %>% 
      mean()
  ) %>% 
  ungroup() %>% 
  mutate(
    baseline = scales::rescale(baseline_av),
...
three_studies = bind_rows(second_panel,third_panel)  
...
\begin{figure}
<<model_figure2, fig=T,echo=F,eval=T, height = 4, width = 13>>=
wrap_elements(grid::textGrob("I'm a \n giant \n turnip")) + p1
@
