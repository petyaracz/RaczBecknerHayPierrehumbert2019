...
gcm_table = celex %>% # take celex
  filter(!is.na(ahpa)) %>% # remove words not in Albright and Hayes celex
  mutate(
    vowels = str_extract_all(ahpa, '[euVoEIi2U{Q34@a6}]'), # match vowels in the albright and hayes transcription
    nvowels = map(vowels, ~ length(.)) # calculate how many vowels the verb has
  ) %>% 
  select(-vowels) %>% # we don't need vowels now
  unnest(cols = c(nvowels)) %>%  # unnest nvowels
  filter(word != 'forsake', nvowels == 1) %>% # drop any verb with more than 1 vowel also "forsake which survives this
  select(category, moder, disc) %>% # keep category, moder, disc columns (= reg/irreg, drove/sang etc, transcription)
  distinct() %>% # repetitions! 
  count(category, moder) %>% # count drove/sang etc across reg/irreg
  mutate( # if it has no category, it's a misc verb
    moder = replace_na(moder, 'other')
  ) %>% 
  pivot_wider( # stretch table
...
reg_count = filter(gcm_table, `verb class` == 'other') %>% 
  pull(`regular set`)
...
print(xtable(gcm_table, digits=2, label="gcmsetsize", caption="Number of forms in each verb class, GCM training data"))
...
mgl_baseline_rules_raw = read_tsv('~/Github/RaczBecknerHayPierrehumbert2019/models/mgl/baseline_mgl/CELEXFull3.sum') %>% 
  mutate(
    form1 = str_replace_all(form1, '»', ''), # get rid of syllable boundary markers
    form2 = str_replace_all(form2, '»', '')
  )

disc_category = test2 %>% # get words and categories from test2. match colnames with above
  distinct(disc, category) %>% 
  mutate(
    form1 = disc
  )
...
disc_reg_past_irreg_past = verb_handles %>% # get reg and irreg transcriptions from verb handles. match colnames with above.
  select(base.form,regular.form,irregular.form) %>% 
  rename(
    form1 = base.form
  )
...
mgl_count = mgl_baseline_rules_raw %>% # clean-up
  mutate(
    `related forms` = str_replace_all(`related forms`, '»', ''),
    exceptions = str_replace_all(exceptions, '»', ''),
    related_list = strsplit(`related forms`, ', '), # split
    exception_list = strsplit(exceptions, ', ')
  )

mgl_count1 = mgl_count %>%
  select(category,rule.type,related_list) %>% 
  unnest(related_list) %>% 
  distinct() %>% 
  count(category, rule.type) %>% 
  mutate(
    type = 'related forms'
  )

mgl_count2 = mgl_count %>%
  select(category,rule.type,exception_list) %>% 
  unnest(exception_list) %>% 
  distinct() %>% 
  count(category, rule.type) %>% 
  mutate(
    type = 'exceptions'
...
mgl_table = rbind(mgl_count1,mgl_count2) %>% 
  pivot_wider(
    names_from = type, values_from = n
  ) %>% 
  arrange(rule.type, category)
  
@

<<mgltable, echo=F, eval=T, results=tex>>=
print(xtable(mgl_table, digits=2, label="mglsetsize", caption="Number of forms in each verb class, MGL training data"))
...
test2 %>% 
  mutate(
    r_baseline_mgl_features = scales::rescale(baseline_mgl_features)
  ) %>% 
  filter(word == 'splive') %>% 
  select(baseline_mgl_features,r_baseline_mgl_features) %>% 
  distinct()
...
mgl_rule_count = mgl_baseline_rules_raw %>% # take raw rules
  filter( # throw out non-category verbs and non reg/irreg rules (in our definition)
    !is.na(category),
    !is.na(rule.type)
  ) %>% 
  mutate( # make a rule column of the form A -> B / C
    rule = paste0(A, ' ->', B, ' / ', Pfeat)
  ) %>% 
  distinct(rule,category,rule.type) %>% # keep DISTINCT rules
  count(category, rule.type) %>%  # count them
  pivot_wider(names_from = rule.type, values_from = n) %>% # stretch table
  mutate( # make row sum
...
<<mgltable, echo=F, eval=T, results=tex>>=
print(xtable(mgl_table, digits=2, label="mglruleses", caption="The number of unique rules posited by the MGL in each verb class"))
...
test2 = test2 %>% 
  mutate(
    verb_baseline_average = baseline_av - mean(baseline_av, na.rm = T),
    participant_prereg_average = pre_reg_av - mean(pre_reg_av, na.rm = T),
    bot_lexical_typicality = case_when(
    ...
    bot_regularisation_rate = reg_rate %>% 
...
lme.fit1 = glmer(resp_post_reg ~ 1 + verb_baseline_average * bot_lexical_typicality + participant_prereg_average + bot_regularisation_rate +
...
save(lme.fit1, file = 'si_fit1.rda')
vif.mer(lme.fit1)
@

<<main_model3, echo=F,eval=T, results=tex>>=
load('si_fit1.rda')
print(xtable(summary(lme.fit1)$coef, digits = 3, label = 'regressiontable1', caption = 'Experimental Data, Post-test Regression Model Summary. Model formula: post-test regular response ~ verb baseline mean x lexical typicality + participant pretest mean + regularization shift + (1 + verb.baseline.mean | participant) + (1 | verb)'))
@
...
fit_power1 = glmer(resp_post_reg ~ 1 + verb_baseline_average * bot_lexical_typicality + participant_prereg_average + bot_regularisation_rate +
    (1 + verb_baseline_average | participant_id) + 
    (1  | word), 
  data = test2b, 
  family = binomial ( link = logit ),
...
fit_power2 = glmer(resp_post_reg ~ 1 + verb_baseline_average + bot_lexical_typicality + participant_prereg_average + bot_regularisation_rate +
    (1 + verb_baseline_average | participant_id) + 
    (1  | word), 
  data = test2b, 
  family = binomial ( link = logit ),
...
mgl_gcm_cor = baseline %>% 
  distinct(word,r_baseline_mgl_features,baseline_gcm_features) %>% 
  summarise(
...
baseline_words = baseline %>% 
  distinct(word, r_baseline_mgl_features, r_baseline_gcm_features)
...
celex_m3 = glmer(regular ~ 1 + r_baseline_mgl_features + r_baseline_gcm_features_residual + 
                (1 + r_baseline_mgl_features + r_baseline_gcm_features_residual | subject), 
                data = baseline, 
                family = binomial ( link = logit ),
                control=glmerControl(optimizer="bobyqa"))

celex_m4 = glmer(regular ~ 1 + r_baseline_gcm_features + r_baseline_mgl_features_residual + 
                (1 + r_baseline_gcm_features + r_baseline_mgl_features_residual | subject), 
                data = baseline, 
                family = binomial ( link = logit ),
                control=glmerControl(optimizer="bobyqa"))

save(celex_m3, file = 'si_celex_m3.rda')
save(celex_m4, file = 'si_celex_m4.rda')

@

<<celex_resid1, echo=F,eval=T, results=tex>>=
load('si_celex_m3.rda')
print(xtable(summary(celex_m3)$coef, digits = 3, label = 'resid1', caption = 'MGL scores and GCM residuals, baseline data'))
@

<<celex_resid2, echo=F,eval=T, results=tex>>=
load('si_celex_m4.rda')
print(xtable(summary(celex_m4)$coef, digits = 3, label = 'resid2', caption = 'GCM scores and MGL residuals, baseline data'))
@
...
test2 = test2 %>% 
  mutate(
    gcm_score_diff = r_individual_gcm_features - r_baseline_gcm_features,
    mgl_score_diff = r_individual_mgl_features - r_baseline_mgl_features
)

resp_post_reg ~ 1 + verb_baseline_average * bot_lexical_typicality + participant_prereg_average + bot_regularisation_rate 

indiv_m5 = glmer(resp_post_reg ~ r_baseline_gcm_features + gcm_score_diff + participant_prereg_average + 
                   (1 + r_baseline_gcm_features + gcm_score_diff | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))

save(indiv_m5, file = 'si_indiv_m5.rda')

indiv_m6 = glmer(resp_post_reg ~ r_baseline_mgl_features + mgl_score_diff + participant_prereg_average  + 
                   (1 + r_baseline_mgl_features + mgl_score_diff | participant_id) + (1 | word), 
                 data = test2, 
                 family = binomial (link = logit), 
                 control=glmerControl(optimizer="bobyqa"))

save(indiv_m6, file = 'si_indiv_m6.rda')

@

<<indiv_diff1, echo=F, eval=T, results=tex>>=

load('si_indiv_m5.rda')
print(xtable(summary(indiv_m5)$coef, digits = 3, label = 'indiv1', caption = 'Celex-GCM and individual score difference, post-test data'))

@

<<indiv_diff2, echo=F,eval=T, results=tex>>=
load('si_indiv_m6.rda')
print(xtable(summary(indiv_m6)$coef, digits = 3, label = 'indiv2', caption = 'Celex-MGL and individual score difference, post-test data'))
@
...
These techniques are indicative of the relative strengths of the individual-MGL and -GCM respective to their \sc{Celex}-counterparts and each other. However, they do not compare them directly using statistical tests. 
